...
def comment(l, indent):
    global sym, val, pos

    newLineComment = False
    if pos == 1:
        newLineComment = True
    ...
    stdout.write(commentPreffix + source[start:index-1] + commentSuffix)
...
def getSym(l = 0, indent = ' '):
    ...
    if ch == chr(0): sym = EOF
        ...
        ident()
    elif ch == '{': comment(l, indent); getSym()
